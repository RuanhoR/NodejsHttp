...
(async()=>{
     await fs.rm(DATA_PATH.verify, {
      recursive: true,
      force: true
    })
    await fs.mkdir(DATA_PATH.verify)
})()
class mail {
  constructor() {
  ...
  async send(mail) {
    try {
      if (!await this.#check(mail).is) throw new Error(
      ...
      const date = dayjs();
      const content = fs_
        .readFileSync(DATA_PATH.mailTip.verify)
        .toString()
  ...
  async ok(mail, code) {
    try {
      let data;
      try {
        data = await get(DATA_PATH.verify, mail);
        if (!(data instanceof Map)) data = new Map();
        const to = Date.now() - parseInt(data.get('time'));
        console.log(to)
        const max_time = 1000 * 60 * 5
        if (
          to >= max_time &&
