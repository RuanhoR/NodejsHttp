...
async function del(t, e, r) {
  return await new Promise(async (i, n) => {
    try {
      let result;
      
      if (r) {
        // 删除特定隐藏文件
        await fs.rm(`${t}/${e}/${r}`, { force: true });
      } else {
        // 删除整个目录（递归删除）
        await fs.rm(`${t}/${e}`, { recursive: true, force: true });
      }
      i({
        code: 200,
        msg: "SUCCESS"
...
async function get(t, e) {
  return await new Promise(async (r, i) => {
    try {
      const dirPath = `${t}/${e}`;
...
async function add(t, e, r) {
  return await new Promise(async (i, n) => {
    try {
      ...
      for (const [fileName, content] of r.entries()) {
        const filePath = `${dirPath}/${fileName}`;
...
async function zipText(t) {
  try {
    return await gzip(Buffer.from(String(t)), { level: 9 });
  } catch (t) {
