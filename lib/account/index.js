"use strict";
const maxAge = 365 * 24 * 60 * 60 * 1000;
const {
  _log, // 统一日志管理函数
  resFile,
  KillBadWord,
  uuid4,
  cors,
  parseCookies,
  mail,
  ToUrl,
  sendmail,
  check,
  HASH_CONFIG,
  DATA_PATH,
  getP,
  Exit
} = require("./../seice");
const fs = require("fs/promises");
const mime = require("mime-types");
const crypto = require("crypto"); // 用于哈希密码
const dayjs = require("dayjs"); // 使用dayjs获取准确时间
const path = require("path")
const {
  get,
  all,
  add,
  del
} = require("./../storage").ButterModule;
/**
 * 账号框架
 */
class account {
  constructor(req, res) {
    this.req = req;
    this.res = res;
    this.cache = null;
  }
  /**
   * 获取用户数据（内部方法）
   */
  async #getUserByName(name, id) {
    if (typeof name !== "string" || name.trim() === "") return new Map();
    if (this.cache?.has(name)) {
      const user = this.cache.get(name);
      if (!id || user.get("id") === id) return user;
      this.cache.delete(name);
    }
    try {
      const [id, mail, password] = await Promise.all([
        get(DATA_PATH.login, name, 'id').catch(() => null),
        get(DATA_PATH.login, name, 'mail').catch(() => null),
        get(DATA_PATH.login, name, 'password').catch(() => null)
      ]);
      const user = new Map([
        ['id', id],
        ['mail', mail],
        ['password', password],
        ['name', name]
      ])
      if (Array.isArray(user) || (id && !user.get("id") === id)) {
        return new Map();
      }
      return user;
    } catch (err) {
      return new Map();
    }
  }
  /**
   * 从 cookie 获取用户信息
   */
  async Get() {
    try {
      const cookie = parseCookies(this.req);
      return await this.#getUserByName(cookie?.name, cookie?.user_id);
    } catch (err) {
      _log(err);
      return new Map();
    }
  }
  /**
   * 校验密码
   */
  async ResUserListFromBody(body) {
    const {
      res
    } = this;
    res.writeHead(200, {
      'Content-type': 'application/json'
    })
    if (this.Get().keys().toArray().length === 0) {
      return res.end(JSON.stringify({
        code: -1,
        msg: 'ERR_NO_log'
      }))
    }
    if (!(body && this.#isValidUsernameFormat(body.name))) {
      return res.end(JSON.stringify({
        code: -1,
        msg: 'ERR_INPUT'
      }));
    }
    const users = await Promise.all([
      all(DATA_PATH.login).catch(() => [])
    ]);
    return res.end(JSON.stringify({
      data: users
        .filter(usr => usr.includes(body.name))
        .map(usr => usr.name),
      code: 200,
      msg: 'SUCCESS'
    }));
  }
  #verifyPassword(password, hash) {
    try {
      if (typeof hash !== 'string') {
        hash = hash.toString();
        if (typeof hash !== 'string') return false
      };
      const [algo, iterations, salt, derived] = hash.split(":");
      const check = crypto.pbkdf2Sync(
        password,
        salt,
        parseInt(iterations),
        HASH_CONFIG.keylen,
        algo
      ).toString("hex");
      return check === derived;
    } catch (err) {
      return false;
    }
  }
  /**
   * 校验密码格式
   */
  #isValidPasswordFormat(str) {
    return /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[^<>]{8,30}$/.test(str);
  }
  /**
   * 校验用户名格式
   */
  #isValidUsernameFormat(str) {
    return /^[\u4e00-\u9fa5a-zA-Z0-9_\-]{2,50}$/.test(str);
  }
  /**
   * 生成加密密码
   */
  #hashPassword(password) {
    const salt = crypto.randomBytes(HASH_CONFIG.saltSize).toString("hex");
    const derived = crypto.pbkdf2Sync(
      password,
      salt,
      HASH_CONFIG.iterations,
      HASH_CONFIG.keylen,
      HASH_CONFIG.algorithm
    ).toString("hex");
    return `${HASH_CONFIG.algorithm}:${HASH_CONFIG.iterations}:${salt}:${derived}`;
  }
  /**
   * 注册或删除用户
   */
  async #registerOrDeleteUser(name, password, mailAddr) {
    try {
      if (password !== 0 || mailAddr) {
        const id = uuid4();
        await add(
          DATA_PATH.login,
          name,
          new Map([
            ["password", this.#hashPassword(password)],
            ["mail", mailAddr],
            ["id", id],
            ["Ctime", dayjs().valueOf()]
          ])
        );
        return id;
      }
      await del(
        DATA_PATH.login,
        name
      );
      return "注销成功";
    } catch (err) {
      _log(err);
      throw err;
    }
  }
  /**
   * 检查是否有重复用户/邮箱
   * @param {{ name: string, mail: string }} e
   * @returns {Promise<boolean>} true 表示没有重复，可以注册
   */
  async #checkDuplicateUser(e) {
    try {
      const users = await all(DATA_PATH.login);
      for (const name of users) {
        const data = await get(DATA_PATH.login, name).catch(() => new Map());
        if (!(data instanceof Map)) continue;
        // 邮箱重复
        if (e.mail && e.mail === data.get("mail")) {
          return false;
        }
        // 用户名重复
        if (e.name.trim() && e.name.trim() === name) {
          return false;
        }
      }
      return true; // 没有重复
    } catch (err) {
      _log(err);
      return false; // 出错时直接拒绝，避免注册脏数据
    }
  }
  /**
   * 登录入口
   */
  async Login(body) {
    const {
      req,
      res
    } = this;
    try {
      this.cache = null;
      const result = await this.handLogin(body);
      res.writeHead(200, {
        "Content-Type": "application/json"
      });
      res.end(JSON.stringify(result));
    } catch (err) {
      _log(err);
      if (!res._header) res.writeHead(200, {
        "Content-Type": "application/json"
      });
      res.end(
        JSON.stringify({
          code: -1,
          msg: err.name
        })
      );
    }
  }
  /**
   * 设置 cookie
   */
  setCookie(cookieStr) {
    this.res.setHeader("Set-Cookie", cookieStr);
  }
  async #setMail(user, body) {
    const {
      res
    } = this;
    try {
      if (!this.hasKeys(
          body, ["verify", "mail"], 2
        ) || body.mail == user.get('mail')) throw new Error("NO_MAIL_VERIFY")
      if (!mail.ok(body.mail, body.verify)) throw new Error("MAIL_VERIFY_ERR")
      const returnValue = await add(
        DATA_PATH.login,
        user.get("name"),
        new Map([
          ["mail", body.mail]
        ])
      );
      res.end(JSON.stringify(returnValue))
      const text = await fs.readFile(DATA_PATH.mailTip.mai, "utf-8")
        .toString()
        .replace("getName()", user.get('name'))
        .replace("mail()", body.mail)
        .replace('omail()', user.get('mail'))
      mail.sendmail({
        html: text,
        from: mail.data.account,
        to: user.get('mail'),
        subject: `你已不是${user.get('name')}的绑定邮箱！`
      })
      setTiimeout(async () => {
        mail.sendmail({
          html: text,
          from: mail.data.account,
          to: user.get('mail'),
          subject: `经过验证码验证，${user.get('name')}已绑定你的邮箱 `
        })
      }, 5000)
    } catch (err) {
      res.end(JSON.stringify({
        code: -1,
        msg: err.message
      }))
    }
  }
  async #setName(user, name) {
    const {
      res
    } = this;
    let rel;
    try {
      if (
        user.get('name').trim() === name.trim() ||
        !this.#isValidUsernameFormat(name.trim())
      ) throw new Error("ERR_usrName_No_chage")
      if (
        !(await this.#checkDuplicateUser({
          name: name
        }))
      ) throw new Error('ERR_INPUT')
      await fs.rename(path.join(DATA_PATH.login, user.get('name')), path.join(DATA_PATH.login, name))
      const expires = new Date(dayjs().valueOf() + maxAge).toUTCString();
      this.setCookie(`name=${encodeURIComponent(name)}; Path=/; Secure; Expires=${expires};`)
      rel = {
        code: 200,
        msg: "SUCCESS"
        // 发送邮件提醒
      };
      const Rtext = await fs.readFile(DATA_PATH.mailTip.name, "utf-8")
      mail.sendmail({
        html: Rtext
          .toString()
          .replace("old_getName()", user.get('name'))
          .replace("new_Name()", name),
        from: mail.data.account,
        to: user.get('mail'),
        subject: "账号名称修改提醒"
      })
    } catch (err) {
      _log(err)
      rel = {
        code: -1,
        msg: err.message
      };
    }
    res.end(JSON.stringify(rel));
  }
  /**
   * 用户自主数据
   */
  async SetSetting(body) {
    const {
      res,
      req
    } = this;
    const user = await this.Get();
    if (!this.hasKeys(body, ["setting", "value"], 2) ||
      /* 
        从cookie获取的用户数据如果
        只有一项，说明没有登录账号
        那就退出逻辑
      */
      user.keys().toArray().length <= 0
    ) {
      res.writeHead(200, {
        "Content-Type": "application/json"
      });
      res.end(
        JSON.stringify({
          msg: "ERR_request_invalid_cookie_or_body",
          code: -1
        })
      );
      return;
    };
    const run = {
      name: () => {
        this.#setName(user, body.value);
      },
      mail: () => {
        this.#setMail(user, body)
      },
      clear: () => {
        this.setCookie([
          "user_id=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT",
          "name=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT"
        ]);
        res.writeHead(200, {
          "Content-Type": "application/json"
        });
        res.end(JSON.stringify({
          msg: "SUCCESS",
          code: 200
        }))
      }
    }
    // 主修改逻辑
    if (run[body.setting]) run[body.setting]();
  }
  /**
   * 判断对象是否包含指定数量的 key
   */
  hasKeys(obj, keys, minValue) {
    if (!(typeof obj === "object" && !Array.isArray(obj))) return false;
    if (!Array.isArray(keys)) return false;
    let count = 0;
    for (const key of keys) {
      if (obj.hasOwnProperty(key)) {
        count++;
        if (count >= minValue) return true;
      }
    }
    return false;
  }
  /**
   * 登录/注册处理
   */
  async handLogin(e) {
    let result = {};
    const user = await this.#getUserByName(e?.name);
    if (e?.name) e.name = e.name.trim();
    const success = () => {
      result.code = 200;
      result.msg = "SUCCESS";
    };
    const setLoginCookie = (name, id) => {
      const expires = new Date(dayjs().valueOf() + maxAge).toUTCString();
      this.setCookie([
        `user_id=${encodeURIComponent(id)}; Path=/; Secure; Expires=${expires}; `,
        `name=${encodeURIComponent(name)}; Path=/; Secure; Expires=${expires}; `
      ]);
    };
    const clearCookie = () => {
      this.setCookie([
        "user_id=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT",
        "name=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT"
      ]);
    };
    // 邮箱登录或注册
    if (["e-log", "reg"].includes(e?.type)) {
      if (!e.mail || !e.name || !this.#isValidUsernameFormat(e.name)) {
        result = {
          code: -1,
          msg: "ERR_INPUT"
        };
        return JSON.stringify(result)
      }
      const veeriftMail = await mail.ok(e.mail, e?.verify)
      if (!veeriftMail) {
        result = {
          code: -1,
          msg: "ERR_INPUT"
        };
        return JSON.stringify(result)
      }
      switch (e?.type) {
        case "e-log":
          success();
          setLoginCookie(e.name, user.get("id"));
          break;
        case "reg":
          if (!await this.#checkDuplicateUser(e)) {
            return {
              code: -1,
              msg: "ERR_user_repeat"
            };
          }
          if (this.#isValidPasswordFormat(e.password)) {
            const id = await this.#registerOrDeleteUser(e.name.trim(), e.password, e.mail);
            setLoginCookie(e.name, id);
            success();
          }
          break;
      }
      // 普通登录/清除/删除
    } else if (
      e?.password &&
      e?.name &&
      this.#verifyPassword(e?.password, user.get('password'))
    ) {
      switch (e?.type) {
        case "log":
          setLoginCookie(e.name, user.get("id"));
          success();
          break;
        case "del":
          await this.#registerOrDeleteUser(e?.name, 0);
          clearCookie();
          success();
          break;
      }
    } else {
      result = {
        code: -1,
        msg: "ERR_INPUT"
      };
    }
    // 如果 result 还是空的
    if (Object.keys(result).length === 0) {
      result = {
        code: -1,
        msg: "ERR_INPUT_TYPE"
      };
    }
    return result;
  }
};
module.exports.account = account;