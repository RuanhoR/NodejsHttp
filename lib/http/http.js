...
async function send(req, res, body, user) {
  try {
    ...
    if (user.has('name')) {
      ...
      fs.writeFile(DATA_PATH.chat,JSON.stringify(file))
      res.writeHead(200, {
...
async function pages(req1, res) {
  try {
    ...
    switch (pathSegments[0]) {
      case "":
        await resFile(req, res, './lib/private-html/index.html', user);
        break;
      case "list":
        if (!isLog) {
          res.writeHead(302, {
          ...
          await resFile(req, res, './lib/private-html/user.html', user);
        }
        break;
      case "log":
        if (!isLog) {
          await resFile(req, res, './lib/private-html/login.html', user);
        } else {
          res.writeHead(302, {
      ...
      case "filesD":
        cors();
        await resFile(req, res, 'html/' + req.url.replace('/filesD/', ""), user);
        break;
      case "favicon.ico":
        cors(); // 开通跨域
        await resFile(req, res, 'html/favicon.ico.png');
        break;
      case "m":
      ...
      case "login":
        if (req.method === 'POST') {
          ...
          req.on("end", async () => {
            ...
            switch (pathSegments[1]) {
              case "verify":
                res.end(JSON.stringify(await mail.send(body.to)));
                break;
              default:
    ...
    res.end(`???\n-http-code:500&&Server-Error-${e.message}`); 
    // 返回 500 状态码
    _log(e)  // 记录
  }
}