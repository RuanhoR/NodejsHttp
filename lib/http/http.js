  ...
  try {
    ...
    switch (pathSegments[0] || "") {
      ...
      case "log":
        let way = '';
        try {
          way = parsedUrl.searchParams.get('source')
        } catch (err) {
          _log(err)
        }
        if (!isLog) {
      ...
      case 'p':
        if (isLog) {
          await serveHtml(_log(`./html/log/${parsedUrl.searchParams.get('page').replace('../','') || 'user'}.html`));
        } else {
          res.writeHead(302, {
            'Location': '/log?source=p?page=' + parsedUrl.searchParams.get('page').replace('../', '')
          });
          res.end();
        }
        break;
