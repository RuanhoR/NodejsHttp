...
async function GetUsers(req) {
  try {
    ...
    const data_All = JSON.parse(fileContent);
    if (cookies.user_id) {
      for (const [name, data] of Object.entries(data_All)) {
        // for 遍历寻找相应项，找不到返回 null
...
async function setLogin(password, name, mail) {
  _log(password, name, mail)
  try {
    const fileContent = await GetFile(DATA_PATH.login);
    let v = {}
    try {
      v = JSON.parse(fileContent) || {}
    } catch (err) {}
    let tip_title;
    if (password === -1) {
      delete v[name];
...
async function Login(body, req, res) {
  try {
    ...
    const isVerifyValid = isOkVerifyCode(v.mail, v.verify, v.id);
      ...
      case "reg": // 注册
        if (
          ...
          Object.values(file).every(
            user => user.mail !== v.mail || user.name !== v.name
          )
        ) {
