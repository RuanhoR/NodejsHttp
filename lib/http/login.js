...
const {
  ...
  getP: getP,
  Exit: Exit
} = require("./config.js"), fs = require("fs/promises"), mime = require("mime-types"), crypto = require("crypto"), dayjs = require('dayjs'), {
  get: get,
  all: all,
  add: add,
  del: del
...
class account {
  ...
  async #r(e, t, s) {
    try {
      if (0 !== t || s) {
        const i = uuid4();
        await add(DATA_PATH.login, e, new Map([
          ["password", this.#i(t)],
          ["mail", s],
          ["id", i],
          ['Ctime', dayjs().valueOf()]
  ...
  async #k(e) {
    try {
      const return_value = await all(DATA_PATH.login).every(async (name) => {
        let data = await get(DATA_PATH.login, name);
        if (data instanceof Map && Object.getPrototypeOf(data) === Map.prototype) data = new Map();
        if (e.mail === data.get('mail') || e.name === name) return false
      })
      return !return_value
    } catch (err) {
      return true
  ...
  async handLogin(e) {
    ...
    const s = await this.#e(e?.name),
      ...
      r = (e, t) => {
        this.setCookie(_log([`user_id=${encodeURIComponent(t)}; Path=/; HttpOnly; `, `name=${encodeURIComponent(e)}; Path=/; HttpOnly; `]))
      },
      a = () => {
    ...
    if (this.cache = null, ["e-log", "reg"].includes(e?.type)) {
      ...
      case "log":
        ...
        break;
      case "clear":
        if (e?.password === s.get('password') && e?.name === s.get('name')) throw new Error('INPUT')
        a(), i();
        break;
      case "del":
