...
class account {
  ...
  async #checkDuplicateUser(e) {
    try {
      const return_value = await all(DATA_PATH.login).every(async (name) => {
        ...
        if (data instanceof Map && Object.getPrototypeOf(data) === Map.prototype) {
          data = new Map();
        }
  ...
  async #setName(user,name) {
    ...
    let rel;
    try {
      await fs.rename(DATA_PATH.login+user.get("name"),name)
      this.setCookie(`name=${encodeURIComponent(name)}; Path=/; HttpOnly;`)
      rel = {
        code: -1,
        msg: "SUCCESS"
      }
  ...
  async handLogin(e) {
    ...
    const setLoginCookie = (name, id) => {
      this.setCookie([
          ...
          `name=${encodeURIComponent(name)}; Path=/; HttpOnly; `
        ]);
    };
    ...
    if (Object.keys(result).length === 0) {
      result = { code: -1, msg: "ERR_INPUT_TYPE" };
    }
